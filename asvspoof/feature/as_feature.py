...
from feature.as_lfcc import lfcc, deltas, lfb
from feature.as_lfcc_baseline import LFCC
from feature.as_lfcc_bp import lfcc_bp

transform_vad = None
lfcc_lcnn_extractor = None
...
def extract_feature(audio, sr, feature_type, access_type=None, extractor=None):
    ...
    #                         nfilts=20, num_ceps=20, order_deltas=2, pre_emph=1, delta_norm=False)
        ...
        if lfcc_lcnn_extractor == None:
            ...
            # fl = 320, fs = 160, fn = 1024, sr = sr, filter_num = 20, with_energy = True, max_freq = 0.5
            lfcc_lcnn_extractor = LFCC(fl=window_len, fs=window_inc, fn=512, sr=sr, filter_num=20,
...
def extract_log_spect(audio, n_fft, win_length, hop_length):
    S, _ = librosa.core.spectrum._spectrogram(audio, n_fft=n_fft, hop_length=hop_length, power=2, win_length=win_length,
                                              window="hann")
...
def extract_lfcc(audio, sr, win_len, hop_len, f_min=0, f_max=4000, num_ceps=20, nfilts=70, nfft=1024, order_deltas=2,
...
def extract_lfcc_bp(audio, sr, win_len, hop_len, f_min=0, f_max=4000, num_ceps=20, nfilts=70, nfft=1024, order_deltas=2,
                    pre_emph=0, delta_norm=True):
...
def extract_lfcc_pt(audio, sr, win_len, hop_len, f_min=0, f_max=4000):
    ...
    extractor = torchaudio.transforms.LFCC(sample_rate=sr, f_min=f_min, f_max=f_max, n_lfcc=20, speckwargs=speckwargs)
    delta = ComputeDeltas()
...
def extract_lfcc_lcnn_baseline(sig, sr, win_len, hop_len, f_min=0, f_max=0.5, num_ceps=20, with_energy=True,
    ...
    if lfcc_lcnn_extractor == None:
        # fl = 320, fs = 160, fn = 1024, sr = sr, filter_num = 20, with_energy = True, max_freq = 0.5
        lfcc_lcnn_extractor = LFCC(fl=win_len, fs=hop_len, fn=1024, sr=sr, filter_num=num_ceps,
                                   with_energy=with_energy, with_emphasis=True,
                                   with_delta=True, flag_for_LFB=flag_for_LFB,
                                   num_coef=None, min_freq=f_min, max_freq=f_max)
    # sig = torch.from_numpy(sig).unsqueeze(0)
    lfcc = lfcc_lcnn_extractor(sig)
    # lfcc = lfcc.squeeze().detach().numpy().T
