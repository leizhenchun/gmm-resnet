...
from asvspoof21.experiment.as19_util import read_protocol
...
def compute_tDCF_EER(cm_scores, access_type, test_type, ds19_dir, figure=False):
    # root_path = '/home/lzc/lzc/ASVspoof2019/DS_10283_3336'

    if isinstance(cm_scores, str):
        cm_scores = pandas.read_csv(cm_scores, sep=' ', header=None, skipinitialspace=True)

    if test_type == 'train':
        test_protocol_file = os.path.join(ds19_dir, access_type,
                                          'ASVspoof2019_{}_cm_protocols'.format(access_type),
                                          'ASVspoof2019.{}.cm.{}.trn.txt'.format(access_type, test_type))
    else:
        test_protocol_file = os.path.join(ds19_dir, access_type,
                                          'ASVspoof2019_{}_cm_protocols'.format(access_type),
                                          'ASVspoof2019.{}.cm.{}.trl.txt'.format(access_type, test_type))
    _, _, _, _, cm_keys = read_protocol(test_protocol_file)

    asv_score_file_pattern = ds19_dir + '/{}/ASVspoof2019_{}_asv_scores/ASVspoof2019.{}.asv.{}.gi.trl.scores.txt'
    asv_score_file = asv_score_file_pattern.format(access_type, access_type, access_type, test_type)
    ...
    # Compute t-DCF
    tDCF_curve, CM_thresholds = compute_tDCF(bona_cm, spoof_cm, Pfa_asv, Pmiss_asv, Pmiss_spoof_asv, cost_model,
                                             False)
    ...
    # print('CM SYSTEM     EER = {:8.5f}%'.format(eer_cm * 100))
    # print('TANDEM   min-tDCF = {:8.5f}'.format(min_tDCF))

    # logging.info('TANDEM   min-tDCF = {:8.5f}'.format(min_tDCF))
    # logging.info('CM SYSTEM     EER = {:8.5f} %'.format(eer_cm * 100))

    logging.info(
        '({}+{})\tmin-tDCF = {:.6f}   EER = {:.6f} %'.format(len(bona_cm), len(spoof_cm), min_tDCF, eer_cm * 100))
...
def compute_tDCF_EER_attack(cm_scores, access_type, test_type, ds19_dir):
    # root_path = '/home/lzc/lzc/ASVspoof2019/DS_10283_3336'
    ...
    if isinstance(cm_scores, str):
        ...
        # Load CM scores
        cm_scores = np.genfromtxt(cm_scores, dtype=str)
        cm_scores = cm_scores[:, 1]
        cm_scores = cm_scores.astype(np.float32)
    ...
    if test_type == 'train':
        ...
        test_protocol_file = os.path.join(ds19_dir, access_type,
                                          'ASVspoof2019_{}_cm_protocols'.format(access_type),
                                          'ASVspoof2019.{}.cm.{}.trl.txt'.format(access_type, test_type))
    _, _, _, cm_attackIds, cm_keys = read_protocol(test_protocol_file)
    ...
    eer_cm, min_tDCF = compute_tDCF_EER_function(tar_asv, non_asv, spoof_asv, bona_cm, spoof_cm)

    idx_bonafide = cm_attackIds == '-'
    cm_keys_bonafide = cm_keys[idx_bonafide]
    cm_scores_bonafide = cm_scores[idx_bonafide]

    attack_list = list(np.unique(cm_attackIds))
    attack_list.remove('-')
    attack_list = sorted(attack_list)
    for attack_id in attack_list:
        logging.info(attack_id)
        idx_attack = cm_attackIds == attack_id
        cm_keys_attack = cm_keys[idx_attack]
        cm_scores_attack = cm_scores[idx_attack]

        compute_tDCF_EER_function(tar_asv, non_asv, spoof_asv, cm_scores_bonafide, cm_scores_attack)


def compute_tDCF_EER_function(tar_asv, non_asv, spoof_asv, bona_cm, spoof_cm):
    # Fix tandem detection cost function (t-DCF) parameters
    Pspoof = 0.05
    cost_model = {
        'Pspoof': Pspoof,  # Prior probability of a spoofing attack
    ...
    logging.info('          min-tDCF = {:8.6f}     EER = {:8.6f} %'.format(min_tDCF, eer_cm * 100))
...
if __name__ == "__main__":
    ...
    compute_tDCF_EER_attack(
        '/home/lzc/lzc/ASVspoof2019/ASVspoof2019exp/GMM_ResNet_2Path_LFCC/20220114_203836_GMM_ResNet_2Path_LFCC_LA/AS19_GMM_ResNet_2Path_LFCC_LA_KEY_eval_ufm_score.txt',
        'LA', 'eval')
