...
from .RawBoost import LnL_convolutive_noise, ISD_additive_noise, SSI_additive_noise, normWav
...
def augment_audio(audio: Tensor, sr, aug_method):
    ...
    if aug_method == 'original':
    ...
    elif aug_method.startswith('rb3'):
        audio = audio.squeeze().numpy()
        augmented_audio = apply_rb3(audio, sr)
        augmented_audio = torch.from_numpy(augmented_audio.astype(numpy.float32)).unsqueeze(0)
        return augmented_audio

    elif aug_method.startswith('rb4'):  # together in series (1+2+3)
        audio = audio.squeeze().numpy()
        augmented_audio = apply_rb1(audio, sr)
        ...
        raise ValueError('aug_method error:' + aug_method)
