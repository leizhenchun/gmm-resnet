...
It includes:
 LFCC: based on experiment.org baseline matlab code
...
class LFCC(torch_nn.Module):
    """ Based on experiment.org baseline Matlab code.
    ...
    def forward(self, x):
        ...
        # filter bank
        # fb_feature = torch.log10(torch.matmul(sp_amp, self.lfcc_fb) + torch.finfo(torch.float32).eps)
        fb_feature = torch.log10(torch.matmul(sp_amp, self.lfcc_fb) + torch.finfo(self.lfcc_fb.dtype).eps)

        # DCT (if necessary, remove DCT)
        lfcc = self.l_dct(fb_feature) if not self.flag_for_LFB else fb_feature
        # if not self.flag_for_LFB:
        #     lfcc = torch.from_numpy(scipy.fft.dct(x=fb_feature.numpy(), type=2, axis=1, norm='ortho'))
        # else:
        #     lfcc = fb_feature
        ...
        if self.with_energy:
            ...
            energy = torch.log10(power_spec.sum(axis=2) + torch.finfo(torch.float32).eps)
            # energy = torch.log10(power_spec.sum(axis=2) + torch.finfo(power_spec.dtype).eps)
            lfcc[:, :, 0] = energy
        ...
        lfcc_output = lfcc_output.permute(0, 2, 1)
