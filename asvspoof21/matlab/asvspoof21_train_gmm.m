...
% gmm_type = 'GMM_F';
% exp_path  = fullfile('/home/labuser/ssd/lzc/ASVspoof/ASVspoof2021exp', [gmm_type, '_', feature_type]);
% make_path(exp_path);
    
% asvspoof_gmm_run_21(feature_type, 'LA', 512, root_path, exp_path, ["Original"]);
% asvspoof_gmm_run_21(feature_type, 'PA', 1024, root_path, exp_path, ["Original"]);

for i = 4 : 4
    aug_methods = ["Original", "RB" + i];
    gmm_type = ['GMM_D_aug2_rb', num2str(i)];
    exp_path  = fullfile('/home/labuser/ssd/lzc/ASVspoof2021exp', [gmm_type, '_', feature_type, '_', datestr(now, 'yyyymmdd_HHMMSS')]);
    make_path(exp_path);
    
    asvspoof_gmm_run_21(feature_type, 'LA', 1024, root_path, exp_path, aug_methods);
%     asvspoof_gmm_run_21(feature_type, 'PA', 1024, root_path, exp_path, aug_methods);
end
...
function asvspoof_gmm_run_21(feature_type, access_type, nmix, root_path, exp_path, aug_methods)

% root_path = '/home/labuser/ssd/lzc/ASVspoof';
% exp_path  = fullfile(root_path, 'ASVspoof2021exp', [gmm_type, '_', feature_type]);
% make_path(exp_path);

copyfile('./asvspoof21_train_gmm.m', fullfile(exp_path, 'asvspoof21_train_gmm.m'));
copyfile('./asvspoof_gmm_diag_em.m', fullfile(exp_path, 'asvspoof_gmm_diag_em.m'));
...
[train_speakerIds, train_fileIds, train_environmentIds, train_attackIds, train_key] = asvspoof19_read_protocol_file( trainProtocolFile );
[dev_speakerIds, dev_fileIds, dev_environmentIds, dev_attackIds, dev_key] = asvspoof19_read_protocol_file( devProtocolFile );
[eval_speakerIds, eval_fileIds, eval_environmentIds, eval_attackIds, eval_key] = asvspoof19_read_protocol_file( evalProtocolFile );
...
[eval_fileIds21, ~] = asvspoof19_read_protocol_file( evalProtocolFile21 );
...
ubm_all = asvspoof_gmm_diag_em(train_feature, nmix, gmm_niter);
...
[eval_DF_fileIds21, ~] = asvspoof19_read_protocol_file( evalDFProtocolFile21 );
...
%     gmm_spoof = gmm_spoof_all{gmm_idx};
    ...
    save_gmm(gmm_filename, ubm.type, ubm.w, ubm.mu, ubm.sigma, ubm_mean, ubm_std);
...
%     save_gmm(gmm_filename, gmm_bonafide.type, gmm_bonafide.w, gmm_bonafide.mu, gmm_bonafide.sigma, bonafide_mean, bonaffide_std);
% 
...
%     save_gmm(gmm_filename, gmm_spoof.type, gmm_spoof.w, gmm_spoof.mu, gmm_spoof.sigma, spoof_mean, spoof_std);
...
function logprob = lgmmprob_diag(data, gmm)
    mu = gmm.mu;
    sigma = gmm.sigma;
    w = gmm.w';
...
function logprob = lgmmprob_full(data, gmm)
    mu = gmm.mu;
    w = gmm.w;
    ...
    logprob = zeros(n_components, n_samples);
    ...
    for idx = 1 : n_components
        ...
        logprob(idx, :) = sum(y.*y, 2);
    end  

end


function post = lgmmprob(data, gmm)
    if gmm.type == 1
        post = lgmmprob_diag(data, gmm);
    elseif gmm.type == 2
        post = lgmmprob_full(data, gmm);
    end
end
...
%         if (ndims(gmm.sigma) == 2)
%             post1 = lgmmprob_diag(feature, gmm);
%         else
%             post1 = lgmmprob_full(feature, gmm);
%         end
        post = lgmmprob(feature, gmm);
        
        feat_x1 = feat_x1 + sum(post, 2)';
        feat_xx1 = feat_xx1 + sum(post .* post, 2)';
...
function save_gmm(gmm_filename, gmmtype, w, mu, sigma, feat_mean, feat_std)


h5create(gmm_filename, '/type', size(gmmtype));
h5write( gmm_filename, '/type', gmmtype);
